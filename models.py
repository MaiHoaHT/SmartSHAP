...
# =========================================================
# FAST (EMBEDDING-CACHED) BI-ENCODER SCORING
# =========================================================
# Instead of re-encoding a freshly concatenated string for every sampled
# coalition (one transformer forward pass per SHAP sample -> the dominant
# cost of ClusterSmartSHAP), we embed each unit exactly once and score a
# coalition as the cosine similarity between the query embedding and the
# mean of the member unit embeddings. This is an APPROXIMATION of jointly
# encoding the concatenated text (bi-encoder cross-attention within a long
# concatenated string is not identical to averaging independently-encoded
# embeddings) -- re-run the eval suite (rank agreement / comprehensiveness)
# after switching this on to confirm quality is preserved for your data.
def _encode_units_cached(model, query, unit_texts, batch_size=32):
    """Encode the query once and every unit once. Returns (query_emb, unit_embs)."""
    query_emb = model.encode_texts([query], batch_size=1, convert_to_numpy=True)
    query_emb = np.asarray(query_emb, dtype=np.float32).reshape(-1)

    unit_embs = model.encode_texts(unit_texts, batch_size=batch_size, convert_to_numpy=True)
    unit_embs = np.asarray(unit_embs, dtype=np.float32)
    if unit_embs.ndim == 1:
        unit_embs = unit_embs.reshape(1, -1)
    return query_emb, unit_embs


def _score_unit_combinations_cached(query_emb, unit_embs, combinations):
    """
    Vectorized coalition scoring from cached embeddings: O(1) matrix ops,
    zero additional transformer forward passes.
    """
    n_combos = len(combinations)
    if n_combos == 0:
        return np.array([], dtype=float)

    dim = unit_embs.shape[1]
    coal_embs = np.zeros((n_combos, dim), dtype=np.float32)
    for row, comb in enumerate(combinations):
        if len(comb) == 0:
            continue  # leaves coal_embs[row] as zeros -> scored as 0.0 similarity
        coal_embs[row] = unit_embs[list(comb)].mean(axis=0)

    q_norm = np.linalg.norm(query_emb)
    if q_norm < 1e-12:
        return np.zeros(n_combos, dtype=float)

    c_norms = np.linalg.norm(coal_embs, axis=1)
    valid = c_norms > 1e-12

    scores = np.zeros(n_combos, dtype=float)
    scores[valid] = (coal_embs[valid] @ query_emb) / (c_norms[valid] * q_norm)
    return scores


def _loo_unit_drops_cached(query_emb, unit_embs):
    """Cached-embedding version of the LOO drop, no extra forward passes."""
    n = unit_embs.shape[0]
    if n == 0:
        return np.array([], dtype=float)

    full_comb = [tuple(range(n))]
    reduced_combs = [tuple(i for i in range(n) if i != j) for j in range(n)]
    all_combs = full_comb + reduced_combs

    scores = _score_unit_combinations_cached(query_emb, unit_embs, all_combs)
    full_score = float(scores[0])
    return full_score - scores[1:]
...
def _smartshap_core_from_units(
    ...
    When scoring="bi_encoder" and use_cached_embeddings=True (default), every
    unit is embedded ONCE and coalition scores are computed as cosine
    similarity against the mean-pooled member embeddings, instead of
    re-encoding a freshly concatenated string per sampled coalition. This
    turns the dominant cost (one transformer forward pass per SHAP sample)
    into a handful of forward passes total plus cheap vector math. It is an
    approximation of jointly encoding the concatenated text -- validate
    against the previous eval numbers before relying on it for the paper.
    Pass precomputed_query_emb/precomputed_unit_embs to reuse embeddings
    computed by a caller (e.g. ClusterSmartShapExplainer) instead of
    re-encoding at every recursion level.
    """
    n = len(unit_texts)
    if n == 0:
        return np.array([], dtype=float)
    ...
    use_fast_path = (scoring == "bi_encoder") and use_cached_embeddings
    if use_fast_path:
        if precomputed_query_emb is not None and precomputed_unit_embs is not None:
            query_emb, unit_embs = precomputed_query_emb, precomputed_unit_embs
        else:
            query_emb, unit_embs = _encode_units_cached(model, query, unit_texts, batch_size=batch_size)
        scores = _score_unit_combinations_cached(query_emb, unit_embs, combinations)
    else:
    ...
    if use_loo_calibration and n > 1:
        if use_fast_path:
            loo_raw = _loo_unit_drops_cached(query_emb, unit_embs)
        else:
...
class ClusterSmartShapExplainer:
    ...
    def __init__(
        ...
        # Encode the query and every sentence exactly once for the whole
        # document. Reused for cluster-level coalition scoring (cluster
        # embedding = mean of member embeddings) and for the recursive
        # per-cluster redistribution -- avoids re-encoding concatenated
        # strings at every SHAP sample and at every recursion level.
        # Only valid for scoring="bi_encoder"; cross_encoder keeps the
        # original (slower, non-cached) path since it needs real query-doc
        # pairs and has no additive embedding structure to exploit.
        self._embeddings_ready = False
        if self.scoring == "bi_encoder" and len(self.sentences) > 0:
            self.query_emb, self.sentence_embs = _encode_units_cached(
                self.model, self.query, self.sentences, batch_size=self.batch_size
            )
    ...
    def _cluster_embeddings(self):
        """Mean-pooled embedding per cluster, reusing cached sentence embeddings."""
        return np.stack([
            self.sentence_embs[members].mean(axis=0)
            for members in self.clusters
        ])

    def _attention_weights(self, member_indices):
        if self._embeddings_ready:
            raw = _score_unit_combinations_cached(
                self.query_emb, self.sentence_embs, [(i,) for i in member_indices]
            )
        else:
    ...
    def _recursive_weights(self, member_indices):
        if self._embeddings_ready:
            member_query_emb = self.query_emb
            member_unit_embs = self.sentence_embs[member_indices]
            member_sents = None
        else:
            member_query_emb = None
            member_unit_embs = None
            member_sents = [self.sentences[i] for i in member_indices]

        raw_coef = _smartshap_core_from_units(
            self.model, self.query, member_sents if member_sents is not None else [self.sentences[i] for i in member_indices],
            ...
            use_cached_embeddings=self._embeddings_ready,
            precomputed_query_emb=member_query_emb,
            precomputed_unit_embs=member_unit_embs,
        )
    ...
    def explain(self, n_samples=None, return_raw=False):
        ...
        cluster_query_emb = self.query_emb if self._embeddings_ready else None
        cluster_unit_embs = self._cluster_embeddings() if self._embeddings_ready else None
        ...
        cluster_raw_coef = _smartshap_core_from_units(
            ...
            return_raw_coef=True,
            use_cached_embeddings=self._embeddings_ready,
            precomputed_query_emb=cluster_query_emb,
            precomputed_unit_embs=cluster_unit_embs,
        )
