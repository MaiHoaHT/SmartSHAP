...
def check_required_files(base_path):
    ...
    if missing_files:
        raise FileNotFoundError(
            "Missed required files in project root: "
            + ", ".join(missing_files)
            + f"\nProject root current: {base_path}"
        )
...
# RESULT SAVING (CSV)
# =========================================================
def save_per_paper_csv(results_dir, mode, results):
    """Save the per-paper detailed results of ONE mode to its own CSV file."""
    if not results:
        return None
    os.makedirs(results_dir, exist_ok=True)
    path = os.path.join(results_dir, f"{mode}_per_paper.csv")
    pd.DataFrame(results).to_csv(path, index=False)
    return path


def update_summary_csv(results_dir, mode, results):
    """
    Append/update ONE row (this mode's averaged metrics) inside a single
    combined summary CSV (results/<run>/summary_all_modes.csv), so every mode
    run via --mode all lands in the same comparison table automatically.
    Reruns of the same mode overwrite its previous row instead of duplicating it.
    """
    os.makedirs(results_dir, exist_ok=True)
    summary_path = os.path.join(results_dir, "summary_all_modes.csv")

    if not results:
        row = {"mode": mode, "n_valid_papers": 0}
    else:
        rank_values = [r["rank"] for r in results if not np.isnan(r["rank"])]
        row = {
            "mode": mode,
            "n_valid_papers": len(results),
            "comp": float(np.mean([r["comp"] for r in results])),
            "suff": float(np.mean([r["suff"] for r in results])),
            "rank": float(np.mean(rank_values)) if rank_values else np.nan,
            "mae": float(np.mean([r["mae"] for r in results])),
            "rmse": float(np.mean([r["rmse"] for r in results])),
            "stability": float(np.mean([r["stab"] for r in results])),
            "runtime_wall": float(np.mean([r["time"] for r in results])),
            "runtime_cpu": float(np.mean([r["cpu_time"] for r in results])),
            "runtime_repeat": float(np.mean([r["repeat_time"] for r in results])),
        }

    if os.path.exists(summary_path):
        summary_df = pd.read_csv(summary_path)
        summary_df = summary_df[summary_df["mode"] != mode]  # replace old row for this mode, if any
...
# RUN ONE MODE
# =========================================================
def run_mode(args, mode, model, loader, evaluator, df_sampled, sample_size, writer, results_dir=None):
    mode = canonical_mode(mode)
    ...
    if results_dir is not None:
        per_paper_path = save_per_paper_csv(results_dir, mode, results)
        summary_path = update_summary_csv(results_dir, mode, results)
        if per_paper_path:
            print(f"[*] Đã lưu chi tiết từng paper: {per_paper_path}")
        print(f"[*] Đã cập nhật bảng tổng hợp:     {summary_path}")
...
def main():
    ...
    results_dir = os.path.join(base_path, "results", exp_folder)
    os.makedirs(results_dir, exist_ok=True)

    if SummaryWriter is None:
        print("[WARN] TensorBoard not found. Skipping TensorBoard logging.")
        writer = NullWriter()
    else:
    ...
    if os.path.exists(summary_path):
        print("\n" + "=" * 78)
        print(f"[*] BẢNG TỔNG HỢP TẤT CẢ MODE ĐÃ CHẠY: {summary_path}")
        print("=" * 78)
